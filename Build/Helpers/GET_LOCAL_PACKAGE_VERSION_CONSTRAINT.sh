...
if (realpath($composerJsonPath) === false) {
    fwrite(
        STDERR,
...
if (isset($composerManifest['extra']['TYPO3-Solr']['stack-for-ci'][$requestedPackageName])) {
    fwrite(STDOUT, $composerManifest['extra']['TYPO3-Solr']['stack-for-ci'][$requestedPackageName] . PHP_EOL);
    exit(0);
}
